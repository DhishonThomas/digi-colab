...
const AccountForm = ({ switchTab, handleFinalSubmit,formData,updateFormData }: any) => {
  const {
    ...
    defaultValues: { email: formData.email||"", password: "", confirm_password: "", otp: "" },
  });
    //|| !formData.files.policeVerification
    if(!formData.files.image  || !formData.files.educationCertificate || !formData.files.bankDocument){
